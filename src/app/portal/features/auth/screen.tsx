import { AuthAdapterError } from "@portal/domains/auth/adapter";
import {
  ...
  normalizePhone221,
  toE164,
} from "@portal/domains/auth/validation";
import {
  clearAuthDraft,
...
import heroImage from "@portal/shared/assets/hero-auth.webp";
import type { PatientProfile } from "@portal/shared/types/flow";
import {
  Activity,
  ArrowLeft,
  ArrowRight,
...
export default function AU01() {
  ...
  const [identityFlowMode, setIdentityFlowMode] =
    useState<IdentityFlowMode>("signup");
  ...
  const [drafts, setDrafts] = useState<AuthDraftStatuses>(() =>
    getDraftStatuses(),
  );
  ...
  const goToStep = useCallback(
    (
      ...
      if (nextStep === "OTP") {
        navigate(
          `/patient-flow/auth/verification?mode=${opts?.otpMode ?? entryMode}`,
          {
            replace: opts?.replace ?? false,
      ...
      if (nextStep === "PIN") {
        navigate(
          `/patient-flow/auth/pin?mode=${opts?.pinRouteMode ?? pinMode}`,
          {
            replace: opts?.replace ?? false,
            state: {
  ...
  useEffect(() => {
    ...
    if (path.endsWith("/auth/connexion")) {
      ...
      const phoneFromState = normalizePhone221(
        locationState.prefillPhone ?? "",
      );
      ...
      const routePending = isPendingAuthState(location.state)
        ? location.state
        : null;
      const storedPending = loadPendingAuthState();
      const resolvedPending =
        [routePending, storedPending]
    ...
    locationState.prefillPhone,
    locationState.inscriptionStep,
    locationState.initialEntryMode,
    phoneNumber,
    navigate,
    incomingFlow,
  ]);
              ...
              <AnimatePresence mode="wait">
                ...
                {step === "DASHBOARD" && (
                  <DashboardStep
                    ...
                    onNotifications={() =>
                      setStatus(
                        "Centre de préférences prêt pour l'intégration API.",
                      )
                    }
                    onLogout={() => {
                      void auth.logout();
    ...
    >
      ...
      <div className="mt-6 text-center space-y-2">
        <p className="text-sm text-[#111214]/40">
          {secondsLeft > 0 ? (
            <span className="font-mono text-[#111214]">
...
function PinStep({
  ...
  const subtitleByMode: Record<PinMode, string> = {
    ...
    login:
      "Entrez votre PIN 4 chiffres pour accéder rapidement à votre compte.",
  ...
  return (
    ...
    >
      ...
      <p className="text-sm text-[#111214]/45 mb-10">
        {formatPhoneE164ToReadable(phoneE164)}
      </p>
    ...
    >
      ...
      <div className="mb-8 rounded-xl border border-[#111214]/10 bg-[#F9FAFB] p-4">
        ...
        <p className="mt-1 text-base font-semibold text-[#111214]">
          ...
          {dateOfBirth ? `• Né(e) le ${dateOfBirth}` : ""}
        </p>
      </div>
...
function DashboardStep({
  ...
  return (
    ...
    >
      ...
      >
        <p
          className="text-[12px] uppercase tracking-[0.16em] text-white/75"
          style={{ fontWeight: 700 }}
        >
          Tableau de bord patient
        </p>
        <h1
          className="mt-2 text-[32px] leading-[1.06]"
          style={{ fontWeight: 620, letterSpacing: "-0.02em" }}
        >
          Bonjour {fullName}
        </h1>
        <p
          ...
          style={{ fontWeight: 500 }}
        >
          Agir pour: <span style={{ fontWeight: 700 }}>{patientLabel}</span> •
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <p
                className="text-[12px] uppercase tracking-[0.12em] text-[#111214]/42"
                style={{ fontWeight: 700 }}
              >
                Prochain rendez-vous
              </p>
              <h2
                className="mt-1 text-[20px] text-[#111214]"
                style={{ fontWeight: 620 }}
              >
                {upcoming.dateLabel} · {upcoming.timeLabel}
              </h2>
              <p
                className="mt-1 text-[13px] text-[#111214]/62"
                style={{ fontWeight: 520 }}
              >
                {upcoming.practitionerName}
              </p>
              <p
                className="text-[13px] text-[#111214]/52"
                style={{ fontWeight: 500 }}
              >
                {upcoming.appointmentType === "video"
                  ? "Consultation vidéo"
                  : "Consultation présentielle"}{" "}
                • {upcoming.locationLabel}
              </p>
            </div>
        ...
        >
          <div className="mb-3 flex items-center justify-between">
            <div>
              ...
              <p
                className="text-[14px] text-[#111214]/62"
                style={{ fontWeight: 520 }}
              >
            ...
            <div
              className="inline-flex items-center gap-1 rounded-full bg-[#F1F6F8] px-3 py-1 text-[12px] text-[#00415E]"
              style={{ fontWeight: 700 }}
            >
        ...
        <div className="rounded-[20px] border border-[#111214]/8 bg-[linear-gradient(138deg,#F8EFE3_0%,#F5EEE4_52%,#EDF4F8_100%)] p-4">
          <p
            className="text-[12px] uppercase tracking-[0.12em] text-[#111214]/42"
            style={{ fontWeight: 700 }}
          >
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <p
            className="text-[12px] text-[#111214]/52"
            style={{ fontWeight: 500 }}
          >
            {subtitle}
          </p>
...
function DashboardKpi({
  ...
  return (
    <div className="rounded-[14px] border border-white/20 bg-white/12 p-2.5 backdrop-blur-[2px]">
      ...
      <p className="text-[11px] text-white/78" style={{ fontWeight: 600 }}>
        {label}
      </p>
    </div>
  );
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5">
        ...
        <p
          className="flex-1 text-[13px] text-[#111214]/78"
...
function TimelineItem({
  ...
  return (
    <div className="flex items-center gap-2.5 rounded-[14px] bg-white/75 px-3 py-2.5">
      ...
      <div>
        <p className="text-[13px] text-[#111214]" style={{ fontWeight: 620 }}>
          {title}
        </p>
        <p
          className="text-[12px] text-[#111214]/50"
          style={{ fontWeight: 500 }}
        >
          {meta}
        </p>
