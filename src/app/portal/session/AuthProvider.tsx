...
import type { AuthFlowContext, AuthSession, AuthUser } from "@portal/session/types";
...
import {
  ...
  normalizeRoles,
  type UserRole,
} from "@portal/session/roles";
import {
  accountAdapter,
  appointmentAdapter,
  authAdapter,
} from "@portal/domains/runtime/adapters";
import {
  readStorageValue,
  removeStorageValue,
  writeStorageValue,
...
function readActingProfileId() {
  return readStorageValue(ACTING_PROFILE_KEY);
}

function writeActingProfileId(profileId: string | null) {
  if (!profileId) {
    removeStorageValue(ACTING_PROFILE_KEY);
    return;
  }
  writeStorageValue(ACTING_PROFILE_KEY, profileId);
}

function readActiveRole(): UserRole | null {
  const raw = readStorageValue(ACTIVE_ROLE_KEY);
  return isUserRole(raw) ? raw : null;
}

function writeActiveRole(role: UserRole | null) {
  if (!role) {
    removeStorageValue(ACTIVE_ROLE_KEY);
    return;
  }
  writeStorageValue(ACTIVE_ROLE_KEY, role);
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const loadProfilesForUser = useCallback(
    async (nextUser: AuthUser) => {
      ...
      try {
        ...
        await accountAdapter.ensureSelfProfile({
          userId: nextUser.id,
  ...
  const refreshConsentSnapshot = useCallback(
    async (profileId?: string | null) => {
      ...
      const consents = await accountAdapter.listConsents(user.id, targetProfileId);
  ...
  const setActingProfile = useCallback(
    async (profileId: string | null) => {
      ...
      if (user) {
        await accountAdapter.recordProfileSwitch(user.id, profileId);
