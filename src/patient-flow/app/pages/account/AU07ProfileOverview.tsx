...
function loadDraftPreConsultSnapshot(): PreConsultSnapshot | null {
  ...
  try {
    const raw = window.localStorage.getItem(PRECONSULT_DRAFT_KEY);
    if (!raw) return null;
    const parsed = JSON.parse(raw) as { data?: unknown; savedAt?: unknown } | null;
